  ...
  Future<void> _verifyOtp() async {
    ...
    if (otp.length < 4) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'الرجاء إدخال الرمز المكون من 4 أرقام على الأقل',
    ...
    try {
      ...
      if (mounted && role != null) {
        if (role == UserRole.courier) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                  'هذا الحساب غير مسجل كمندوب في طلباتي',
        ...
        context.go(
          '/register',
          extra: {'phone': widget.phone, 'email': widget.email},
        );
      }
    } catch (e) {
      if (mounted) {
