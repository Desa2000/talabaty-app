  ...
  void _onSearchChanged(String query) {
    ...
    setState(() {
      ...
      _filteredStores = _allStores.where((s) {
        final matchFilter =
            _activeFilter == 'all' || s.type.toString().contains(_activeFilter);
        return matchFilter &&
            (s.name.toLowerCase().contains(lq) ||
                s.area.toLowerCase().contains(lq));
      }).toList();
      _filteredProducts = dp.products
          .where(
            (p) =>
        ...
        body: SafeArea(
          child: Column(
            children: [
              ...
              Expanded(child: _isSearching ? _buildResults() : _buildBrowse()),
            ],
          ),
        ),
      ),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(20),
          border: Border.all(
            color: active ? AppColors.primaryColor : const Color(0xFFDDDDDD),
          ),
        ),
        child: Text(
          label,
          style: TextStyle(
            ...
            fontSize: 12,
            fontWeight: FontWeight.w700,
            color: active ? AppColors.primaryColor : const Color(0xFF666666),
          ),
        ),
      ),
    );
    ...
    return ListView(
      ...
      children: [
        if (_filteredStores.isNotEmpty) ...[
          Text(
            'المطاعم والمتاجر (${_filteredStores.length})',
            style: const TextStyle(
              fontFamily: 'Cairo',
              fontWeight: FontWeight.w900,
              fontSize: 16,
          ...
          const SizedBox(height: 12),
          ..._filteredStores.asMap().entries.map(
            (e) => _storeResultCard(e.value)
                .animate()
                .fade(
                  duration: 250.ms,
        ...
        if (_filteredProducts.isNotEmpty) ...[
          ...
          ..._filteredProducts
              ...
              .asMap()
              .entries
              .map((e) => _productResultCard(e.value)),
        ],
      ],
    );
  }
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 3),
                      Text(
                        'توصيل ${store.deliveryFee.toStringAsFixed(0)} ج.س',
                        style: const TextStyle(
                          fontFamily: 'Cairo',
                          color: Color(0xFF888888),
                          fontSize: 12,
                      ...
                      Container(
                        ...
                        child: Text(
                          store.status == 'active' ? 'مفتوح' : 'مغلق',
                          style: TextStyle(
                            fontFamily: 'Cairo',
                            color: statusColor,
                            fontSize: 11,
          ...
          children: [
            ClipRRect(
              ...
              child: SizedBox(
                width: 56,
                height: 56,
                child: CustomImage(imagePath: product.image, fit: BoxFit.cover),
              ),
            ),
            const SizedBox(width: 12),
            Expanded(
