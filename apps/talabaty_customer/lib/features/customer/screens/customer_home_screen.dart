...
import 'package:talabaty_customer/core/providers/data_provider.dart';
import 'package:talabaty_customer/core/providers/auth_provider.dart';
import 'package:talabaty_customer/core/constants/app_colors.dart';
import 'package:talabaty_customer/data/models/store_model.dart';
import 'package:talabaty_customer/core/constants/enums.dart';
import 'package:talabaty_customer/core/services/delivery_fee_service.dart';
...
class _CustomerHomeScreenState extends State<CustomerHomeScreen> {
  ...
  String _distanceLabel(StoreModel store) {
    ...
    final km = DeliveryFeeService.distanceKm(
      _userLat!,
      _userLng!,
      store.latitude,
      store.longitude,
  ...
  String _deliveryFeeLabel(StoreModel store) {
    if (_userLat == null || _userLng == null) return '';
    final fee = DeliveryFeeService.deliveryFeeFromCoords(
      _userLat!,
      _userLng!,
      store.latitude,
      store.longitude,
  ...
  List<StoreModel> _sortedByDistance(List<StoreModel> stores) {
    ...
    list.sort((a, b) {
      final da = DeliveryFeeService.distanceKm(
        _userLat!,
        _userLng!,
        a.latitude,
        a.longitude,
      );
      final db = DeliveryFeeService.distanceKm(
        _userLat!,
        _userLng!,
        b.latitude,
        b.longitude,
  ...
  Widget build(BuildContext context) {
    ...
    final restaurants = _sortedByDistance(
      stores.where((s) => s.type == StoreType.restaurant).toList(),
    );
    final supermarkets = _sortedByDistance(
      stores.where((s) => s.type == StoreType.supermarket).toList(),
    );
    final pharmacies = _sortedByDistance(
      stores.where((s) => s.type == StoreType.pharmacy).toList(),
    );
      ...
      body: SafeArea(
        child: CustomScrollView(
          ...
          slivers: [
            ...
            SliverToBoxAdapter(
              child: _buildHorizontalStoreList(context, restaurants),
            ),
            SliverToBoxAdapter(
              child: _buildSectionTitle(
                'السوبرماركت',
            ...
            SliverPadding(
              ...
              sliver: SliverList(
                delegate: SliverChildBuilderDelegate(
                  (context, index) =>
                      _buildVerticalStoreCard(context, nearby[index]),
