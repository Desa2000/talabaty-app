...
class _MerchantMenuTabState extends State<MerchantMenuTab> {
  ...
  Widget build(BuildContext context) {
    ...
    final merchantId = auth.currentUser != null
        ? 'store_${auth.currentUser!.id}'
        : 's1';
    final products = dataProvider.products
        .where((p) => p.storeId == merchantId)
        .toList();
          ...
          IconButton(
            ...
            onPressed: () {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('قريباً: البحث في المنيو')),
              );
            },
          ),
        ],
                    ...
                    onChanged: (val) {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(
                            val
                                ? 'تم إعادة المنتج للقائمة بنجاح'
                                : 'تم إيقاف المنتج من الظهور للعملاء',
                          ),
