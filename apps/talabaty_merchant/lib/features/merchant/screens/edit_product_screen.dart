import 'package:talabaty_merchant/core/widgets/custom_image.dart';
import 'dart:io';
                  ...
                  children: [
                    TextField(
                      decoration: const InputDecoration(
                        labelText: 'اسم المجموعة (مثال: اختر الحجم)',
                      ),
                    ...
                    ...tempOptions.map(
                      (opt) => ListTile(
                        ...
                        trailing: IconButton(
                          ...
                          onPressed: () =>
                              setDialogState(() => tempOptions.remove(opt)),
                        ),
                      ),
                    ),
                    ElevatedButton.icon(
                          ...
                          builder: (_) => AlertDialog(
                            ...
                            content: Column(
                              ...
                              children: [
                                TextField(
                                  decoration: const InputDecoration(
                                    labelText: 'اسم الخيار (مثال: كبير)',
                                  ),
                                ...
                                TextField(
                                  ...
                                  onChanged: (v) =>
                                      optPrice = double.tryParse(v) ?? 0.0,
                                ),
                              ],
                            ),
                            actions: [
      ...
      builder: (ctx) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              TextField(
                decoration: const InputDecoration(labelText: 'اسم الملحق'),
                onChanged: (v) => addOnName = v,
              ),
              TextField(
                decoration: const InputDecoration(labelText: 'السعر الإضافي'),
                keyboardType: TextInputType.number,
                onChanged: (v) => addOnPrice = double.tryParse(v) ?? 0.0,
              ),
            ],
          ),
          actions: [
            ...
            children: [
              ...
              _buildCard(
                child: Column(
                  ...
                  children: [
                    Text(
                      'مجموعات الخيارات الحالية: ${_optionGroups.length}',
                    ...
                    ..._optionGroups.map(
                      (group) => Card(
                        ...
                        child: ListTile(
                          title: Text(
                            '${group.name} (${group.isRequired ? 'إجباري' : 'اختياري'})',
                          ),
                          subtitle: Text('${group.options.length} خيارات'),
                          trailing: IconButton(
                            icon: const Icon(Icons.delete, color: Colors.red),
                            onPressed: () =>
                                setState(() => _optionGroups.remove(group)),
                          ),
                        ),
                      ),
                    ),
                    ...
                    Text(
                      'الإضافات الحالية: ${_addOns.length}',
                      style: const TextStyle(fontWeight: FontWeight.bold),
                    ),
                    const SizedBox(height: 8),
                    Wrap(
                      ...
                      children: _addOns
                          .map(
                            (addon) => Chip(
                              label: Text(
                                '${addon.name} (+${addon.price} ج.س)',
                              ),
                              deleteIcon: const Icon(Icons.close, size: 18),
                              onDeleted: () =>
                                  setState(() => _addOns.remove(addon)),
                            ),
                          )
                          .toList(),
                    ),
