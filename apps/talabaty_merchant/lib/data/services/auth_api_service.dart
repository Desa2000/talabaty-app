...
class AuthApiService {
  ...
  Future<Map<String, dynamic>> registerCustomer({
    ...
    try {
      ...
      throw e.error is ApiException
          ...
          : ApiException(message: e.message ?? 'فشلت عملية التسجيل');
    }
  }
  ...
  Future<Map<String, dynamic>> login({
    ...
    try {
      ...
      throw e.error is ApiException
          ? e.error as ApiException
          : ApiException(message: e.message ?? 'فشل تسجيل الدخول');
    }
  }

  Future<Map<String, dynamic>> otpLogin({required String identifier}) async {
    try {
      final response = await _apiClient.dio.post(
        '/api/auth/otp-login',
